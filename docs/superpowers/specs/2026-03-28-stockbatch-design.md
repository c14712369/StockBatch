...
- `doGet(e)`：接收 cron-job.org 的 HTTP 請求，驗證 secret token 後呼叫 `runStockScan()`
- `runStockScan()`：主流程
...
- **不使用 GAS 內建時間觸發器**（誤差 ±15 分鐘，不符合需求）
- **改用 cron-job.org**（同 FoodBatch / AvBatch 模式）：
  - cron 設定：`30 8 * * 1-5`（台北時間週一至週五 08:30 整）
  - 呼叫 GAS `doGet` endpoint 觸發 `runStockScan()`
  - GAS 端驗證 secret token，防止未授權呼叫
- `setupTrigger()` 函式不需要，改由 `doGet` 接收外部呼叫
...
| `CRON_SECRET` | GAS PropertiesService + cron-job.org | cron-job.org 呼叫 doGet 的驗證 token |
